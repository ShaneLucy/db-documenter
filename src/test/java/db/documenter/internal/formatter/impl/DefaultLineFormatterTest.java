...
class DefaultLineFormatterTest {

  private DefaultLineFormatter defaultLineFormatter;
  private Table table;
  private Column.Builder columnBuilder;

  @BeforeEach
  void setUp() {
    defaultLineFormatter = new DefaultLineFormatter();
  ...
  class FormatTests {
    ...
    void ifCurrentIsNotNullItReturnsCurrent() {
      ...
      final var result = defaultLineFormatter.format(table, Column.builder().build(), currentValue);
    ...
    void ifColumnMaximumLengthGreaterThan0ThenReturnStringIsFormattedCorrectly() {
      ...
      final var result = defaultLineFormatter.format(table, column, null);
    ...
    void ifTableIsNullItDoesNotThrowException() {
      assertDoesNotThrow(() -> defaultLineFormatter.format(null, columnBuilder.build(), "current"));
    }

    @Test
    void ifCurrentIsNullItDoesNotThrowException() {
      assertDoesNotThrow(() -> defaultLineFormatter.format(table, columnBuilder.build(), null));
    }

    @Test
    void ifCurrentIsEmptyStringItReturnsEmptyString() {
      final var result = defaultLineFormatter.format(table, columnBuilder.build(), "");
      assertEquals("", result);
    }
    ...
    void ifColumnNameIsNullDoesNotThrow() {
      ...
      assertDoesNotThrow(() -> defaultLineFormatter.format(table, column, null));
    }
    ...
    void ifColumnIsNullThrowsNullPointerException() {
      assertThrows(
          NullPointerException.class, () -> defaultLineFormatter.format(table, null, null));
    }

    @Test
    void ifTableIsNullAndCurrentIsNullDoesNotThrow() {
      final var column = columnBuilder.maximumLength(10).build();
      assertDoesNotThrow(() -> defaultLineFormatter.format(null, column, null));
    }
  }
}
