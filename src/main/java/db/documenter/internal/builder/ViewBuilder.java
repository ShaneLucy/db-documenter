 ...
 *
 * <p><b>Column Enrichment:</b> Column metadata is retrieved via {@link
 * QueryRunner#getColumnInfo(String, String)}, which works for tables, views, and materialized views
 * because all expose columns through {@code information_schema.columns}. USER-DEFINED column types
 * are resolved using the same {@link ColumnMapper} logic used for tables.
 *
...
public final class ViewBuilder {
   ...
   * QueryRunner#getMaterializedViewInfo(String)}, this method fetches its columns and resolves any
   * USER-DEFINED types to enum names. No primary key or foreign key queries are performed because
   * materialized views cannot have those constraints in PostgreSQL.
   *
  ...
  public List<MaterializedView> buildMaterializedViews(
    ...
    for (final MaterializedView stubMatView : stubMatViews) {
      final List<Column> rawColumns = queryRunner.getColumnInfo(schema, stubMatView.name());
